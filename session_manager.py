...
def _load_sessions() -> List[Session]:
    # Known Session field names — filter JSON to only these so that old or
    # future-format files never cause Session(**s) to raise TypeError.
    ...
    try:
        ...
        sessions = []
        for s in raw:
            # Supply defaults for every field we know about
        ...
        import traceback
        traceback.print_exc()   # surface errors instead of silently swallowing
        return []
...
def _save_sessions(sessions: List[Session]):
    ...
    # Debug: print what we're saving so we can verify fields are present
    for s in data:
        print(
            f"[_save_sessions] {s['name']}: "
            f"buttons={len(s.get('buttons', []))}, "
            f"font_size={s.get('font_size', '?')}",
            file=sys.stderr,
        )
...
class _SessionEditor(QDialog):
    ...
    def __init__(self, session: Optional[Session] = None, parent=None):
        ...
        for w in (self._name, self._host):
